---
name: jarvis
description: Use when the user asks Claude Code to read or write Jarvis data (todos, email, calendar, memory, documents) through the scoped Claude Agent API. Requires JARVIS_URL and JARVIS_API_TOKEN.
---

# Jarvis

Use this skill when a user asks to interact with Jarvis from Claude Code.

## Configuration

Expect these environment variables:

- `JARVIS_URL`: Base URL for the user's Jarvis instance, for example `http://127.0.0.1:7000`.
- `JARVIS_API_TOKEN`: Scoped API token created in Jarvis Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in Jarvis Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in Jarvis settings.
...
- All Jarvis data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write Jarvis user data.
...
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py todos list
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py todos add "Follow up"
```
...
```bash
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py emails list 5
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py emails read UID
```
...
```bash
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py GET /api/codex/memory
python3 ~/.claude/skills/jarvis/scripts/jarvis_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
If you are about to reach the Jarvis host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Jarvis Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.
