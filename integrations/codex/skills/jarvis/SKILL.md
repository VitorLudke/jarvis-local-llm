---
name: jarvis
description: Use when the user asks Codex to read or write Jarvis data from a terminal Codex session through the scoped Codex Agent API. Requires JARVIS_URL and JARVIS_API_TOKEN.
---

# Jarvis

Use this skill when a user asks to interact with Jarvis from Codex.
...
- `JARVIS_API_TOKEN`: Scoped API token created in Jarvis Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in Jarvis Settings and expose both values to the terminal session.
...
- All Jarvis data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/jarvis_api.py capabilities
python3 integrations/codex/scripts/jarvis_api.py todos list
python3 integrations/codex/scripts/jarvis_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/jarvis_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 integrations/codex/scripts/jarvis_api.py emails list 5
python3 integrations/codex/scripts/jarvis_api.py emails read UID
```
...
```bash
python3 integrations/codex/scripts/jarvis_api.py GET /api/codex/memory
python3 integrations/codex/scripts/jarvis_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
If you are about to reach the Jarvis host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Jarvis Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
