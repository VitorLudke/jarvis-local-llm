                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Jarvis (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Jarvis" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "jarvis_kind" if "jarvis_kind" in cols else "'scheduled' AS jarvis_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                jarvis_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Jarvis-Origin"] = "jarvis-ui"
                outer["X-Jarvis-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", jarvis_kind or "scheduled")[:64]
                outer["X-Jarvis-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `jarvis-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `JARVIS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `jarvis-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("JARVIS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `JARVIS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (JARVIS_INPROCESS_POLLERS=0); "
            "drive `jarvis-mail poll-scheduled` externally."
        )
        return
    import asyncio
