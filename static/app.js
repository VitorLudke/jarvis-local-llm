// ============================================
// Jarvis UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__jarvisDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Jarvis Chat';
  ...
  if (_opener) window._jarvisRouteOpener = _opener;
  ...
  fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    .then(d => {
      ...
      // The display_name pref (Settings → Account) overrides the auth
      // username — the only identity in single-user mode (auth off).
      fetch(`${API_BASE}/api/prefs/display_name`, { credentials: 'same-origin' })
        .then(r => r.json())
        .then(p => {
          if (p && p.value) {
            if (userBarName) userBarName.textContent = p.value;
            if (userBarAvatar) userBarAvatar.textContent = String(p.value).charAt(0).toUpperCase();
          }
        }).catch(() => {});
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('jarvis-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'jarvis-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Jarvis...');
      }
  ...
  const UI_VIS_KEY = 'jarvis-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'jarvis-toolbar-visibility';
    try {
...
// ============================================
function startJarvisApp() {
  if (window.__jarvisAppStarted) return;
  window.__jarvisAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('jarvis-doc-open-' + _curSession) === '1') {
  ...
  window._jarvisBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._jarvisRouteOpener) {
          try { window._jarvisRouteOpener(); } catch (_) {}
          window._jarvisRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startJarvisApp, { once: true });
} else {
  startJarvisApp();
}
