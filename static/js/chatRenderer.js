...
const _ACTION_RECENTS_KEY = 'jarvis-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'jarvis-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Jarvis';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Jarvis' : shortModel(resolvedModel);
