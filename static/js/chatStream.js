...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.jarvis-highlight').forEach(function(e) { e.classList.remove('jarvis-highlight'); });
      document.querySelectorAll('.jarvis-hl-label').forEach(function(e) { e.remove(); });
