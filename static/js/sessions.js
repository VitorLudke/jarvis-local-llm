...
let _sortMode = Storage.get('jarvis-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Jarvis Chat';
...
const FOLDER_STATE_KEY = 'jarvis-folder-state';
const FOLDER_ORDER_KEY = 'jarvis-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Jarvis kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Jarvis Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('jarvis-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('jarvis-session-sort', mode);
  else Storage.remove('jarvis-session-sort');
  renderSessionList();
}
