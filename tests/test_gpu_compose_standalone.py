...
base compose with only the matching overlay merged into the ``jarvis``
...
# --- jarvis = base service + only the overlay additions ------------------


def test_nvidia_jarvis_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_jarvis_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
